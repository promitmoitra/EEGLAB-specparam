...
function fooof_results = fooof(freqs, power_spectrum, f_range, setting, return_model)
    ...
    alpha_band = py.list([8,13]); alpha_power = py.fooof.utils.trim_spectrum(py_freqs,py_power_spectrum,alpha_band);
    lbeta_band = py.list([13,20]);lbeta_power = py.fooof.utils.trim_spectrum(py_freqs,py_power_spectrum,lbeta_band);
    hbeta_band = py.list([20,30]);hbeta_power = py.fooof.utils.trim_spectrum(py_freqs,py_power_spectrum,hbeta_band);
    gamma_band = py.list([30,45]);gamma_power = py.fooof.utils.trim_spectrum(py_freqs,py_power_spectrum,gamma_band);
    ...
    fooof_results.bandpowers = [trapz(double(delta_power{1}),double(delta_power{2}));
                                ...
                                trapz(double(lbeta_power{1}),double(lbeta_power{2}));
                                trapz(double(hbeta_power{1}),double(hbeta_power{2}));
...
%                                          trapz(double(lbeta_power{1}),double(lbeta_power{2}))-trapz(double(lbeta_power{1}),param_ap_fit(double(lbeta_power{1})));
%                                          trapz(double(hbeta_power{1}),double(hbeta_power{2}))-trapz(double(hbeta_power{1}),param_ap_fit(double(hbeta_power{1})));
